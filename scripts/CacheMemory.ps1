...
function Get-RegistryValue
{
    ...
    try{
        ...
        #If the registry doesn`t exist, it will throw an exception
        return $null
    }
}
...
function Get-CacheSize
{
    ...
    switch($Level){
        ...
        default {throw "This is not a cache level, please use [1|2|3|all]"}
    }
...
switch($Operation){
    ...
    default{throw "Not a valid operation. Use -Operation [ getsize --> Get size of select cache memory level (KB) | enable --> Enable the selected cache memory level | disable --> Enable the selected cache memory level | getstatus --> See if the selected cache memory level(s) are enabled or not ]"}
}
